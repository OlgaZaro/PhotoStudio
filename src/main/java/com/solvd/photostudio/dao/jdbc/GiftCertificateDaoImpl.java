...
public class GiftCertificateDaoImpl implements GiftCertificateDao {
    ...
    public void insert(GiftCertificate giftCertificate) {
        try {
            prepareStatement = getPrepareStatement("INSERT INTO giftCertificate "
    ...
    public Optional<GiftCertificate> getById(Integer id) {
        try {
            prepareStatement = ConnectionUtil.getPrepareStatement("SELECT * FROM giftCertificate WHERE id = ?");
    ...
    public List<GiftCertificate> getAll() {
        ...
        try {
            prepareStatement = ConnectionUtil.getPrepareStatement("SELECT * FROM giftCertificate",
    ...
    public void update(GiftCertificate giftCertificate, int id) {
        try {
            prepareStatement = ConnectionUtil.getPrepareStatement("UPDATE giftCertificate SET certificatePrice = ? ," +
    ...
    public void delete(int id) {
        try {
            prepareStatement = ConnectionUtil.getPrepareStatement("DELETE FROM giftCertificate " + "WHERE id = ?",
                    Statement.RETURN_GENERATED_KEYS);
            prepareStatement.setInt(1, id);
